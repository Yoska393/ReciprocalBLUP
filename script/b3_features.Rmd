...
```{r}
feat <- get_selected_features("0.1")  # 閾値0.1のデータを取得
dcg <- feat$dcg
ccg <- feat$ccg
dc  <- feat$dc
cc  <- feat$cc
...
features_list <- list(
  ...
  o = setdiff(intersect(C, D), union(A, B))  # kと同じ場合もある
)
...
## 列スコア: ベクトルは要素値、行列/DFは「列最大値」をスコアに
...
## 各 nm（例: "G_MET_W4"）のスコア（named numeric）を作る
...
## 出力オブジェクト：上流 → percent → drought/control
met_sets <- list(
...
# 例: met_sets$BOTH$p50$drought
# 例: met_sets$G_MET$p12_5$control

## 保存（任意）
saveRDS(met_sets, here("data","percentile_met_sets_G_COM_BOTH.rds"))

## --- micro（G_COM, MET_COM, BOTH）を作る ---
...
# 出力オブジェクト：上流 → percent → 下流（drought/control）
micro_sets <- list(
