...
f_list <- readRDS(here::here("data", "threshold_metabo.rds"))
...
for (thr in names(met2_sub_list)) {
  submat <- met2_sub_list[[thr]]
  covmat <- cov_list[[thr]]
...
for (rep_idx in 1:n_repeat) {
  ...
  for (thr in thresholds) {
    # --- K（固定）と K（ランダム）を取得 ---
    K_fixed <- met2_sub_cov_bundle$covs[[thr]]
K_rand  <- rand_met2_list[[thr]][[rep_idx]]$cov
...
saveRDS(
  ...
  here::here("out", paste0("rainbow_cor_summary_", CD, ".rds"))
)

# 必要ならCSV（しきい値ごとに）
dir.create(here::here("out", "rainbow_cor_csv", CD), recursive = TRUE, showWarnings = FALSE)
for (thr in thresholds) {
  if (!is.null(cor_fixed_summary10[[thr]]))
    write.csv(cor_fixed_summary10[[thr]],
              here::here("out", "rainbow_cor_csv", CD, paste0("fixed_", thr, ".csv")),
              row.names = FALSE)
  if (!is.null(cor_rand_summary10[[thr]]))
    write.csv(cor_rand_summary10[[thr]],
              here::here("out", "rainbow_cor_csv", CD, paste0("rand_", thr, ".csv")),
              row.names = FALSE)
}
...
read
```{r, message =F,include=FALSE}
cor_all<-readRDS(here("out",paste0("rainbow_cor_summary_", CD, ".rds")))
...
print(ite)
t<-paste0("thr",names(met2_sub_cov_bundle$covs)[ite])
