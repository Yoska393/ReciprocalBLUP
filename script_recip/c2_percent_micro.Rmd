...
f_list  <- readRDS( here("data","percentile_micro_sets_G_METCOM_BOTH.rds"))

models  <- names(f_list)               # "G_COM"   "MET_COM" "BOTH"
percent <- c("100","50","25","12_5","6_25")
thresholds <- paste0(percent)          # keep the same names for later intersect
n_repeat <- 10
...
# num_met is expected to be a named integer vector (p100, p50, ...)
if (is.list(num_met)) {
...
## 1) Build “fixed K” per model (random will be created once later)

## =========================================================

fixed_covs <- setNames(vector("list", length(models)), models)  # fixed_covs[[model]][[per]] = K
fixed_counts <- setNames(vector("list", length(models)), models) # number of columns per threshold (record for random baseline)
...
# keep only cov with the same structure for convenience
...
# container for results: cor_summary[[candidate]][[per]][[rep_idx]] = data.frame(...)
...
for (per in percent) {
  ...
  K_list <- list(
    G_COM   = fixed_covs[["G_COM"]][[per]],
    MET_COM = fixed_covs[["MET_COM"]][[per]],
...
## - per_repeat: raw results by candidate → threshold → repeat

## - summary10 : aggregated results by candidate → threshold (e.g., cor_mean_mean)
...
here::here("out", paste0("rainbow_cor_summary_", CD, "_4cands_micro.rds"))
)
}
...
# 2) Safeguard K (symmetrize and ensure positive semi-definiteness)
...
# 3) Use the existing run_rainbow_cv() as is

# (seedInd is prepared in your code)
...
# 5) (Optional) Merge into an existing result object for easier plotting/comparison later
