...
　rng <- c(rng[1] - pad, rng[2] + pad)
  ...
  open_png(out_file, width = 1200, height = 1200, res = 150)
   ...
   plot_title <- if (kind == "micro") {
    "Model1: Genome -> Micro / Model3: Met -> Micro"
  } else {
    "Model2: Genome -> Met / Model4: Micro -> Met"
    }
...
ggh4x::facet_grid2(
  ...
  switch = "y"  # y strip move to left
    ...
    theme(
      panel.grid  = element_blank(),
      plot.title  = element_text(hjust = 0.5, size = 18, face = "bold"),
      strip.text  = element_text(size = 10),  # black
      axis.text   = element_text(size = 8)
    ) +
...
make_pairs_ggplot <- function(kind, CDs, groups, outdir = here("out/hist")){
  ...
  plot_title <- if (kind == "micro") {
    "Prediction of Microbiome (from Genome or Metabolome)"
  } else {
    "Prediction of Metabolome (from Genome or Microbiome)"
    }
